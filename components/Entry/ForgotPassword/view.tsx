...
import { Box, Heading, Input, FormControl, FormLabel, Button, Text, Divider, Badge, Spinner } from '@chakra-ui/react';
import { ErrorMessage } from '@hookform/error-message';
import { ComponentPropTypes, PreviewTypes } from './types';

const View: React.FC<ComponentPropTypes> = ({ formState, onSubmit, previewType }) =>
  previewType === PreviewTypes.LOADING ? (
    <Box alignItems="center" display="flex" justifyContent="center" h="100vh" w="100vw">
      <Spinner colorScheme="darcula.600" size="lg" />
    </Box>
  ) : (
    <Box alignItems="center" display="flex" justifyContent="center" h="100vh" w="100vw" bgColor="gray.50">
      ...
      >
        ...
        <form onSubmit={formState.handleSubmit(onSubmit)} className="form__full-width">
          {previewType === PreviewTypes.SEND_EMAIL ? (
            <Controller
              name="email"
          ...
          <Button mt={1} mb={3} isLoading={formState.formLoading} type="submit" width="full" variant="darcula">
            {previewType === PreviewTypes.SEND_EMAIL ? 'SEND' : 'CHANGE PASSWORD'}
          </Button>
